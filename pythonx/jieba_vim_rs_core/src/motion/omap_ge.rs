...
/// Test if a token is stoppable for `omap_ge`.
...
impl<C: JiebaPlaceholder> WordMotion<C> {
    ...
    /// it and reuse most code from `nmap ge`.
    ///
...
mod tests {
    ...
    #[vcase(name = "newline_newline", buffer = ["}", "{"], count = 2, prevent_change)]
    ...
    #[vcase(name = "newline_one_word", buffer = ["}", "aaa{a"], count = 2, prevent_change)]
    ...
    #[vcase(name = "newline_space_one_word", buffer = ["}", "   aaa{a"], count = 2, prevent_change)]
    ...
    #[vcase(name = "newline_space_newline_one_word", buffer = ["}", "  ", "", "aa{a"], count = 3, prevent_change)]
    ...
    mod motion_omap_c_ge {}

    // Copied from omap_c_ge above.
    #[verified_cases(
        mode = "o",
        operator = "y",
        motion = "ge",
